      program velest
c
c     version 4.1  E.Kissling, Institute of Geophysics, ETH Zurich
c
c                  20. January 2011   
c
c  extended and updated in collaboration with Thomas vanStiphout 
c  (ETH Zuerich) and Andrea Bruestle (Uni Bochum)
c
c  this version is running as: 
c                           
c                               -  velest.f for linux (edited datetime func by codex)
c
c     compile simply by command:  gfortran -std=legacy velest.f -o velest (slow)
c     compile simply by command:  ifx velest.f -o velest (fast)
c
c   this version updated to run in simultaneous mode for 2500 events
c
c    the program is based on the version 3.0 HRM,UK,EK,and W.Ellsworth
c    of 28.oct93, on version 3.1 EK 21April95, and on version 3.3 EK
c    21.August98.
...
c
c    version 4.0
cEK adapted for LINUX and tested by Thomas vanStiphout 2007
cEK  to run in simultaneous mode with 2500 events
cEK
c
c    version 4.1
cEK adapted for format errors and inclusion of new switch to change
cEK  weight given to obs-classes  by Andrea Bruestle 2011
c
c   in velest.cmn - file:
c   *********************
c
c new switch  IQC:  (Quality Classes)
c (applicable only if input is in *.CNV format)
c
c   iqc=0   quality classes as defined in input*.CNV-file    (default)
c   iqc=1   quality classes may be redefined for each obs
c           (iwt0, iwt1,...)
c                             see subr. INPUTCNV
c
c
c new switch  IQF:   (Quality Factors)
c (applicable only if input is in *.CNV format)
c  obs. weight factor defined for each obs. quality class
c
c   iqf=0    as before by default in VELEST (HYPO71, USGS code):
c            1/(2**(obs. weight)*2))
c            (i.e., iwf0=1,iwf1=0.5,iwf2=0.25,iwf3=0.125,
c            obs. weight >=4 are being neglected)
c
c   iqf=1    obs. weight factor separately defined for each class
c            (f.e.,  iwf0=1,iwf1=0.75,iwf2=0.5,iwf3=0.25,iwf4=0.125)
c
c                            see subr. INPUTDATA 
c
c
c  ******************************************
c                                          
...
csh 07/10/2003
c  comment call to CPUTIMER due to portability problems in g77
      cpusec=0
c
...
c
 ...
 59         format(' Final rms values: ',10f5.2,/,(19x,10f5.2))
            write(16,'(//)')
cab   begin  
            write(16,'('' rms values > 2*RMS RESIDUAL: '')')
            do i=1,neqs
            if(rms(i).ge.(2*sqrt(xmsqrs1)))then
            write(16,'(5x,''Event'',1x,i4,'' -> '',f5.2)') i,rms(i)
            endif
            enddo
            write(16,*)
cab   end
...
c
      ...
      character*40 titl
cab  line(32) -> line(34)
      character*80 card,line(34), titleline
      logical lexist
c
...
c
 ...
 111  read(10,'(a)',end=222) card
      ...
      i=i+1
cab   gt.32) -> gt.34)
      if(i.gt.34) stop'INPUTPARAM>>> control-file not correct!'
      line(i)=card
      goto 111
 222  continue
cab   eq.32.)) -> eg.34.))
      if(.NOT.(i.eq.26.or.i.eq.34.))
...
cab   begin
      read(line(12),*) iqc,iwt0,iwt1,iwt2,iwt3,iwt4,iwt5
      read(line(13),*) iqf,iwf0,iwf1,iwf2,iwf3,iwf4
cab   end
      read(line(14),'(a)') modelfilename
      read(line(15),'(a)') stationfilename
      read(line(16),'(a)') seismofilename
      read(line(17),'(a)') regnamfile
      read(line(18),'(a)') regkoordfile
      read(line(19),'(a)') topo1file
      read(line(20),'(a)') topo2file
      read(line(21),'(a)') phasefile
      read(line(22),'(a)') shotfile
c     Output files:
      read(line(23),'(a)') outfile
      if(outfile.eq.' ') outfile='vel.out'
      read(line(24),'(a)') velfile
      if(velfile.eq.' ') velfile='velout.vel'
      read(line(25),'(a)') cnvfile
      if(cnvfile.eq.' ') cnvfile='velout.cnv'
      read(line(26),'(a)') stafile
...
cVMS         open(16,file=outfile,status='new',carriagecontrol='list')
     ...
     &              ''EARTHQUAKES for simult. inversion IEQ = '',i4)')
     &              ieq
         write(16,'('' Max. number of '',
...
c
     ...
     &                            irflout,irfrout,iresout
      ...
      write(16,'(a)')'***'
cabek   begin 
      write(16,'(a)')'  obs. quality classes, iqc=0 default,',
     &     ' as in HYPO71'
      write(16,'(a)')'  if iqc=1 redefining of classes while ',
     &     'input reading'
      write(16,'(a)')'*** iqc   iwt0  iwt1  iwt2  iwt3  iwt4  iwt5' 
      write(16,'(5x,i1,5x,i1,5x,i1,5x,i1,5x,i1,5x,i1,5x,i1)') iqc,
     &                              iwt0,iwt1,iwt2,iwt3,iwt4,iwt5
      write(16,'(a)')'  '
      write(16,'(a)')'  weights attributed to obs. quality classes '
      write(16,'(a)')'  iqf=0 default, as in HYPO71, iqf=1 new ',
     &     'weights def.'
      write(16,'(a)')'*** iqf   iwf0  iwf1  iwf2  iwf3  iwf4'
      write(16,'(5x,i1,4x,f4.2,2x,f4.2,2x,f4.2,2x,f4.2,2x'//
     &      ',f4.2)') iqf,iwf0,iwf1,iwf2,iwf3,iwf4
cabek   end
      write(16,'(a)')'***'
...
c
      do 15 j=1,nobsread
c
cEK first filter all observations: for simultaneous mode only
cEK  accept P-wave obs-classes      0 to 3
c
      if(isingle.eq.0)then
         if(ipwt(j).ge.4)then     ! do not accept phase-weights >= 4  !!!
            goto 15               !  for simultaneous mode
         endif
      else                        ! for single-event-mode
...
c
c remember, for simultaneous mode only classes 0 - 3 accepted, see filter
c                                      in top part of do-loop 15
cabek Jan2011:
cab  begin default or specially defined weighting factor for classes
cab  iqf=0 use default observation weighting factor (0-4) 
cab  iqf=1 newly defined observation weighting factors
c
      if(iqf.eq.1)then
         if(ipwt(j).eq.0) w(nobs,i)=iwf0
         if(ipwt(j).eq.1) w(nobs,i)=iwf1
         if(ipwt(j).eq.2) w(nobs,i)=iwf2
         if(ipwt(j).eq.3) w(nobs,i)=iwf3
         if(ipwt(j).ge.4) w(nobs,i)=iwf4
      else
c  default USGS HYPO71-weights
...
c
c  *****************************************************
c                         end loop  for each observation
c
...
cek print P and S number of obs
           if(nsp.eq.2) then
            write(16,'(i4,1x,3i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,
...
c
      ...
      write(16,*)
cab   (16,'(5 -> (16,'(1
      write(16,'(1(''  Event# -> GAP''))')
cab   (16,'(5(5x,i3 -> (16,'(1(5x,i4
      write(16,'(1(5x,i4,'' -> '',i3))') (i,igap(i),i=1,neqs)
      write(16,*)
      mini=361
      maxi=-1
      iavgap=0
     ...
     &           nint(mini),nint(maxi)
      ...
      write(16,*)
cab   begin write gaps >= 180°
      write(16,'(1(''  Events with gap >= 180°''))')
      do i=1,neqs
      if(igap(i).ge.180)then
      write(16,'(1(5x,i4,'' -> '',i3))') i,igap(i)
      endif
      enddo
      write(16,*)
cab   end write gaps >= 180°
c
      write(16,*)
      write(16,*)
...
cc         write(16,*)
      ...
      if(mge5i.gt.0)then
cab  > 100.  ->  > 170.
         write(16,'(1x,''DEPTH ( input ) > 170. '',''km :  '',a)')
     &              cstari(1:mge5i)
      else
cab  > 100.  ->  > 170.
         write(16,'(1x,''DEPTH ( input ) > 170. '',''km :  '',a)')
     &              ' '
      ...
      if(mge5l.gt.0)then
cab  > 100.  ->  > 170.
         write(16,'(1x,''DEPTH (last_IT) > 170. '',''km :  '',a)')
     &              cstarl(1:mge5l)
      else
cab  > 100.  ->  > 170.
         write(16,'(1x,''DEPTH (last_IT) > 170. '',''km :  '',a)')
...
c
      if(.not.single_turbo)then
cab   (1x,i3 -> (1x,i4
         write(16,'(1x,i4,1x,3i2.2,1x,2i2.2,f6.2,
...
c
cek     write(7,'(3i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,2f7.2)')
...
cek
      write(7,'(3i2.2,1x,2i2.2,1x,f5.2,1x,f7.4,a1,1x,f8.4,a1,2f7.2,
     &         4x,i3,5x,f5.2)')
     &         iyr(i),imo(i),iday(i),ihr(i),nin,sec,
     &         xlat,cns,xlon,cew,
...
c
      ...
      integer j,j1,j2
      integer iqc,iwt0,iwt1,iwt2,iwt3,iwt4,iwt5
c
  1   eventtype='L'
      j2=-1
      nobs=0
...
c
   99 continue
      ...
      enddo
cEKAB Jan 2011 modify quality classes for observations?
c
cab begin 
cab use orginal observation weight (iqc=0) or modify observation weights (1)
         if(iqc.eq.1)then
          do j=1,nobs
              if(iwt(j).eq.0) iwt(j)=iwt0
              if(iwt(j).eq.1) iwt(j)=iwt1
              if(iwt(j).eq.2) iwt(j)=iwt2
              if(iwt(j).eq.3) iwt(j)=iwt3
              if(iwt(j).eq.4) iwt(j)=iwt4
              if(iwt(j).eq.5) iwt(j)=iwt5
          enddo
         endif
cab end
cEKAB Jan 2011 end
...
c
     ...
     &            sta(j),cphase(j),iwt(j),ttime(j)
         if(sta(j).eq.' ')then
cek     mistake:       j1=j1-1 ! event is completely read-in
            goto 99
         endif
         j1=j
      enddo
...
cab   Event# '',i3 -> Event# '',i4
               write(16,'('' Event# '',i4,'' >>> '',1x,3i2.2,1x,2i2.2,
     &                    ''  AVRES ='',f6.2,'' NOBS ='',i3)')
...
cab   included 2nd part for P only!
         else
           nofreadp=nofreadp+nactualsta(i)
           nofreads=0
         endif
cab   end
      enddo
     ...
     &           nstaeff
         write(16,*)
         write(16,*) 
         write(16,'(''  output sub. actualstations:   '')')
         write(16,'('' Total number of readings:   '',i10)') nofreadings
         write(16,'('' Total number of P readings: '',i10)') nofreadp
         write(16,'('' Total number of S readings: '',i10)') nofreads
         write(16,*)
      endif
c
      RETURN
...
cab  2f8.4)') -> 2f10.4)') 
         write(16,'(1x,a4,3x,a1,i4,3f8.4,1x,2f10.4)') 
...
c comment subroutine cputime out because of portability problems to g77
c  SH 07/10/2003
c      subroutine CPUTIMER(cpusec)   ! Urs Kradolfer, 16.9.91
c
...
c
c      implicit none
c      integer icpu, clock
c      real cpusec
c      external clock !$pragma C(clock)
c
c      icpu=clock()
c      cpusec=float(icpu)/1000000.
c
c      end ! of subroutine cputimer
c
...
c
csh 07/10/2003
c     modifications to be compatible w/ g77
c
      implicit none
      ...
      integer :: values(8)
c
c     Use the portable Fortran intrinsic instead of Unix-specific
c     time()/ctime() calls.
c
      call date_and_time(cdate, ctime, czone, values)
      dattim = cdate(5:8)//'-'//cdate(1:4)//' '//ctime(1:8)
c
